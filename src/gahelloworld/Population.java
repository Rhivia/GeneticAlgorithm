...
public class Population {
	 ...
	 */
	public Population(int size, float crossoverRatio, float elitismRatio, float mutationRatio, float invertionRatio) {
		
		this.crossover = crossoverRatio;
                this.invertion = invertionRatio;
	...
	public void evolve() {
		...
		while (idx < buffer.length) {
			...
			} else if (rand.nextFloat() <= invertion) {
				
                                if (rand.nextFloat() <= mutation) {
					buffer[idx] = popArr[idx].invert();
				} else {
